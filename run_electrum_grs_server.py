...
if os.path.dirname(os.path.realpath(__file__)) == os.getcwd():
    imp.load_module('electrumgrsserver', *imp.find_module('src'))
  

from electrumgrsserver import storage, networks, utils
from electrumgrsserver.processor import Dispatcher, print_log
from electrumgrsserver.server_processor import ServerProcessor
from electrumgrsserver.blockchain_processor import BlockchainProcessor
from electrumgrsserver.stratum_tcp import TcpServer
from electrumgrsserver.stratum_http import HttpServer
...
def create_config(filename=None):
    ...
    config.set('server', 'banner', 'Welcome to Electrum-GRS!')
    config.set('server', 'banner_file', '/etc/electrum-grs.banner')
    ...
    if not filename:
        for path in ('/etc/', ''):
            filename = path + 'electrum-grs.conf'
            if os.path.isfile(filename):
                break

    if not os.path.isfile(filename):
        print 'could not find electrum-grs configuration file "%s"' % filename
        sys.exit(1)
